...
<header>
    ...
    <main>
        <?php if (!$patient): ?>
            <div class="search-container" style="max-width:400px;margin:40px auto;padding:32px 24px;background:#fff;border-radius:12px;box-shadow:0 2px 16px rgba(0,0,0,0.07);">
                <h2 style="text-align:center;margin-bottom:20px;color:#000;">Search for a Patient</h2>
                <form method="POST" action="billing.php" style="display:flex;flex-direction:column;gap:16px;">
                    <input type="text" name="search" placeholder="Enter patient ID or name" required style="padding:10px 12px;border:1px solid #bfc9d1;border-radius:6px;font-size:16px;">
                    <button type="submit" style="padding:10px 0;background:#78f15f;color:#fff;border:none;border-radius:6px;font-size:16px;cursor:pointer;transition:background 0.2s;">Search</button>
                </form>
            </div>
        <?php else: ?>
            ...
            <div class="medicine-container">
                ...
                <form id="medicine-form">
                    <div class="input-group">
                        ...
                        <select name="medicine" id="medicine" onchange="updateMedicinePrice()" required style="width:100%">
                            ...
                            while ($medicine = $medicineResult->fetch_assoc()) {
                                $brand = $medicine['brand_name'] ? $medicine['brand_name'] : 'No Brand';
                                echo "<option value='{$medicine['item_id']}' data-price='{$medicine['unit_price']}' data-stock='{$medicine['quantity']}'>
                                        {$medicine['item_name']} - {$brand} (Stock: {$medicine['quantity']})
                                    </option>";
                            }
                            ?>
                        </select>
