...
$lowStockQuery = "
    SELECT i.item_id, i.item_name, i.quantity,
        (SELECT MAX(s.sale_date) FROM sales s WHERE s.item_name = i.item_name) AS last_sold
    FROM inventory i
    WHERE i.quantity < 50
";
$lowStockResult = $conn->query($lowStockQuery);
while ($row = $lowStockResult->fetch_assoc()) {
    $lowStockNotifications[] = $row;
}
?>
      ...
      <section class="profile-navbar">
        <div class="profile-info">
            ...
            <div class="profile-actions">
                <button id="notification-toggle" class="profile-link" style="border: 2px solid #75f15f; background-color: #75f15f; color: white; margin-right: 7 0px;">Notifications</button>
                        ...
                        <?php
                            ...
                            if (!empty($lowStockNotifications)) {
                                foreach ($lowStockNotifications as $item) {
                                    echo '<li>';
                                    echo '<div>';
                                    echo '<strong style="color:#a00;">Low Stock:</strong> ';
                                    echo "The stock for item '<b>" . htmlspecialchars($item['item_name']) . "</b>' (ID: " . $item['item_id'] . ") is low. Only <b>" . $item['quantity'] . "</b> left in inventory.";
                                    if ($item['last_sold']) {
                                        $lastSold = new DateTime($item['last_sold']);
                                        $now = new DateTime();
                                        $diff = $now->diff($lastSold);
                                        if ($diff->d > 0) {
                                        $ago = $diff->d . " day(s) ago";
                                        } elseif ($diff->h > 0) {
                                        $ago = $diff->h . " hour(s) ago";
                                        } elseif ($diff->i > 0) {
                                        $ago = $diff->i . " minute(s) ago";
                                        } else {
                                        $ago = "just now";
                                        }
                                        echo "<br><small>Last sold: " . htmlspecialchars($item['last_sold']) . " ($ago)</small>";
                                    }
                                    echo '</div></li>';
                                }
                            }
                            ...
                            if ($notifications->num_rows > 0) {
                                ...
                                if ($notification['type'] == 'low_stock') continue; 
                                echo '<li>';
                                echo '<a href="#announcement-' . $notification['announcement_id'] . '" class="notification-link">';
                                echo '<strong>';
                                ...
                                echo '</strong>';
                                echo '<small>' . time_elapsed_string($notification['created_at']) . '</small>';
                                echo '<div>';
                                echo '<strong>' . $notification['title'] . '</strong>';
                                echo '<p>' . $notification['message'] . '</p>';
                                echo '</div>';
                                echo '</a></li>';
                                }
                            }
                            if (empty($lowStockNotifications) && $notifications->num_rows == 0) {
        ...
        <div class="container">
          <?php if ($_SESSION['role'] == 'Doctor'): ?>
              ...
              <?php while ($announcement = $pending_announcements->fetch_assoc()): ?>
                  <div class="announcement">
                      <h3><?php echo $announcement['title']; ?></h3>
                      <p><?php echo $announcement['content']; ?></p>
                      <small>Posted by <?php echo $announcement['user_name']; ?> on <?php echo $announcement['created_at']; ?></small>
                      <form action="" method="post">
                          <input type="hidden" name="announcement_id" value="<?php echo $announcement['id']; ?>">
                          <button type="submit" name="approve_announcement">Approve</button>
                      </form>
                  </div>
              <?php endwhile; ?>
          <?php endif; ?>
                            ...
                            <?php if (!empty($announcement['id'])): ?>
                                <div class="announcement">
                                    <a href="#announcement-<?php echo htmlspecialchars($announcement['id']); ?>" class="filter-result-link">
                                        ...
                                        <h4 style="color:black;"><?php echo htmlspecialchars($announcement['title']); ?></h4>
          ...
          <?php while ($announcement = $announcements->fetch_assoc()): ?>
            <div class="announcement" id="announcement-<?php echo $announcement['id']; ?>">
                ...
                ?>
                    ...
                    <?php endwhile; ?>
                    <small><a href="dd.php" class="go-to-top">Go Back to Top</a></small>
            </div>
          <?php endwhile; ?>
        </div>
    </main>
