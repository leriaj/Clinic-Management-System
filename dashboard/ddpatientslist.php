...
if (isset($_GET['update_status'])) {
    ...
    if ($checkResult && $checkResult->num_rows > 0) {
        ...
        if ($conn->query($updateQuery) === TRUE) {
            echo "<script>alert('Status updated to Completed'); window.location.href = 'ddpatientslist.php';</script>";
        } else {
            echo "Error updating status: " . $conn->error;
        }
    } else {
        echo "<script>alert('Please complete all patient details before updating the status.'); window.location.href = 'ddpatientslist.php';</script>";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Patient registered successfully!'); window.location.href='ddpatientslist.php';</script>";
    } else {
    ...
    <main>
        ...
        <div class="c1">
            ...
            <div class="search-bar">
                <form method="GET" action="ddpatientslist.php">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if (strtolower($row["status"]) === "pending" && $isComplete) {
                            $statusCell .= "<a href='ddpatientslist.php?update_status={$row["patient_id"]}' style='color:green;text-decoration:underline;cursor:pointer;'>Pending</a>";
                        } else {
                        ...
                        $rowHtml = "<tr>
                            ...
                            <td>{$row["time"]}</td>
                            $statusCell
                            <td><a href='pprof.php?patient_id={$row["patient_id"]}' class='update-btn'>{$actionLabel}</a></td>
                        </tr>";
